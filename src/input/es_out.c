...
static void EsOutDecodersStopBuffering( es_out_t *out, bool b_forced )
{
    ...
    //input_clock_ChangeSystemOrigin( p_sys->p_pgrm->p_clock, true, i_current_date + i_wakeup_delay - i_buffering_duration );
    //now clock has it's auto algorithm to calculate network jitter/decoder latency, stream to system time convert no need add p_sys->i_pts_delay.
